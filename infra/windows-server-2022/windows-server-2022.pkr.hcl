# SYNOPSIS:
# 	This build file will generate a new packer build on windows server 2022
...
source "vsphere-iso" "win-server2022" {
  ...
  CPUs                 = 4
  RAM                  = 8192
  // Disabling CPU/ RAM hotplug to ensure NUMA awareness
  // CPU_hot_plug         = true
  // RAM_hot_plug         = true
...
build {
  name    = "win-server2022"
  sources = ["source.vsphere-iso.win-server2022"]
  ...
  provisioner "powershell" {
    ...
    inline = [
      ...
      "while($true) { try { $imageState = Get-ItemProperty HKLM:\\SOFTWARE\\Microsoft\\Windows\\CurrentVersion\\Setup\\State | Select ImageState; Write-Output $imageState.ImageState; if($imageState.ImageState -eq 'IMAGE_STATE_GENERALIZE_RESEAL_TO_OOBE') { break }} Catch {}; Start-Sleep -s 10 }"
    ]
  }
